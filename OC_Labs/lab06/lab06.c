...
int main(int argc, char* argv[])
{
	...
	else if (childId > 0) //процесс-родитель
	{
		...
		int status = 0;
		int m = 1;
		while (waitpid(-1, &status, 0 | WNOHANG) == 0) //Ждём, пока не завершится процесс ребёнок, не блокируя процесс родитель, если процес ребёнок кем-то заблокирован. Пока процесс ребёнок не завершился, но кем-то заблокирован - делаем. WHOHANG - если процесс ребёнок в состоянии ожидания - тогда не wait
		{
			printf("Step %i: ", m);
			for (int i = 0; i < N; i++)
			{
